package com.tata.selenium.test.DestinationCost;
...
public class TC_02_DestinationCostCSVExportValidation implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_02_DestinationCostCSVExportValidation.class.getName());
	String properties =  "./data/DestinationCost.properties";
	...
	public void DO (String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest("Execution triggered for - "+TC_02_DestinationCostCSVExportValidation.class.getName()+" - "+uniqueDataId);
		String sheetName="DestinationCost_Screen";
		...
		if(!dataMap.get("CountryList").isEmpty())
		{
			cu.clickElement("DestinationCost_CountryListButton");
			for(String currCountry : dataMap.get("CountryList").split("\\~"))
			{
				cu.setData("DestinationCost_CountryListSearchTxtBox", currCountry);
				cu.sleep(500);
				cu.selectCheckBox("DestinationCost_CountryList_Dynamic_Checkbox", "$country$", currCountry);
			}
			cu.clickElement("DestinationCost_CountryLabel");
			cu.waitForPageLoadWithSleep("", 500);
			
			cu.clickElement("DestinationCost_DestinationListButton");
			if(!dataMap.get("DestinationList").isEmpty() && !dataMap.get("DestinationList").equals("ALL"))
			{
				for(String currCountry : dataMap.get("DestinationList").split("\\~"))
				{
					cu.setData("DestinationCost_DestinationListSearchTxtBox", currCountry);
					cu.sleep(500);
					cu.selectCheckBox("DestinationCost_DestinationList_Dynamic_Checkbox", "$destination$", currCountry);
				}
				cu.clickElement("DestinationCost_DestinationLabel");
			}
			
		}
		
	
		cu.clickElement("DestinationCost_DisplayButton");
		cu.waitForPageLoadWithSleep("", 500);
	  ...
	  public void beforeMethod(String testCaseId) throws Exception {
		  ...
		  extent = ExtReport.instance("Destination_Cost");
	  }	
	...
	{
		List<String> allHeaderNames = cu.ElementsToListWithTrim("DestinationCost_Table_AllHeaders");
		int tableRowSize = cu.ElementsToList("DestinationCost_Table_AllDataRows").size();
		...
		for(int i=1;i<tableRowSize+1;i++)
		{
			 ...
			 List<String> curRowColoumTxts = cu.ElementsToListWithTrim("DestinationCost_Table_Dynamic_Row_AllColoumn", "$index$", i+"");
	...
	public void exportCSVAndValidateWithUI(CommonUtils cu,List<Map<String, String>> uiDataMapRows)  
	  {
		  	...
		  	cu.clickElement("DestinationCost_ExportCSVLink");
			cu.waitForPageLoad("");
			cu.sleep(2000);
			String csvFilePath = cu.getDownlaodedFileName();
			String fileName ="DestinationCost";
