...
public class TC_03_EditRule implements ApplicationConstants {
	
	private static final Logger LOGGER = Logger.getLogger(TC_03_EditRule.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) {
		...
		test = extent.startTest(
				"Execution triggered for - "+TC_03_EditRule.class.getName()+" -with TestdataId: " + uniqueDataId);
				...
				cu.clickElement("filteringRules_dynamic_Row_EditButton_BasedOn_RuleID", "$ruleid$", ruleId);				
				...
				if(!dataMap.get("SubRules_Json").trim().isEmpty())
				{
					...
					{						
						String subRuleIndex =null;
						switch (subRule.get("ChangeType").toLowerCase()) {
						case "add":
							cu.clickElement("filteringRules_EditableRow_SubRule_AddSubRuleButton");
							cu.sleep(500);
							subRuleIndex= "last()";
							addSubRule(subRule, subRuleIndex);
							break;

						case "edit":
							subRuleIndex = getSubRuleIndex(subRule, ruleId);
							if(subRuleIndex!=null)
								modifySubRule(subRule, subRuleIndex);
							break;
							
						case "statuschange":
							subRuleIndex = getSubRuleIndex(subRule, ruleId);
							if(subRuleIndex!=null)
								changeSubRuleStatus(subRule, subRuleIndex);
							break;
							
						case "delete":
							subRuleIndex = getSubRuleIndex(subRule, ruleId);
							if(subRuleIndex!=null)
								deleteSubRule(subRule, subRuleIndex);
							break;
						}											
					}
				}
