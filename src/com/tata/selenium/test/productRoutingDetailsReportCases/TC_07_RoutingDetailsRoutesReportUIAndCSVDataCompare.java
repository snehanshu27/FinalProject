...
public class TC_07_RoutingDetailsRoutesReportUIAndCSVDataCompare implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_07_RoutingDetailsRoutesReportUIAndCSVDataCompare.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest(
				"Execution triggered for - TC_07_RoutingDetailsRoutesReportUIAndCSVDataCompare -with TestdataId: " + uniqueDataId);
		...
		List<List<String>> allMainRowsCSVData = exportCSVAndGetData(cu, "Routes.csv");
		...
		for(int i=0;i<allMainRowsSize; i++)
		{
			...
			if(currentColumnUIMainData.size()==7)
			{	
				String indexNoMainUI = currentColumnUIMainData.get(0);
				String productMainUI = currentColumnUIMainData.get(4);
				String noOfRoutesMainUI = currentColumnUIMainData.get(5);
				...
				if(!noOfRoutesMainUI.isEmpty() && !noOfRoutesMainUI.equals("0"))
				{
					...
					List<List<String>> allDrilldownCSVRowsData = exportCSVAndGetData(cu, "Routes - Details - "+drillDownSelectedProductUI+".csv");
					
					//Drilldown table compare
					if(compareUIAndCSV(allDrilldownUIRowsData, allDrilldownCSVRowsData))
						test.log(LogStatus.PASS,
								"EXPECTECD: (Drilldown) Both UI and exported CSV Drilldown data should be macthed. (mainindex: "+indexNoMainUI+" ^ Product: "+productMainUI+")", 
