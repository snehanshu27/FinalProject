...
public class TC_06_RoutingDetailsPerformanceReportUIAndCSVDataCompare implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_06_RoutingDetailsPerformanceReportUIAndCSVDataCompare.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest(
				"Execution triggered for - TC_06_RoutingDetailsPerformanceReportUIAndCSVDataCompare -with TestdataId: " + uniqueDataId);
		...
		List<List<String>> allMainRowsCSVData = exportCSVAndGetData(cu, "Performance.csv");
		...
		for(int i=0;i<allMainRowsSize; i++)
		{
			...
			if(currentColumnUIMainData.size()==6)
			{	
				//Drilldown
				...
				List<List<String>> allDrilldownCSVRowsData = exportCSVAndGetData(cu, "Performance - Details.csv");
				
				//Drilldown table compare
				if(compareUIAndCSV(allDrilldownUIRowsData, allDrilldownCSVRowsData))
					test.log(LogStatus.PASS,
							"EXPECTECD: (Drilldown) Both UI and exported CSV Drilldown data should be macthed. (mainindex: "+(i+1)+")", 
