...
public class TC_05_RoutingDetailsDistributionReportValidations implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_05_RoutingDetailsDistributionReportValidations.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest(
				"Execution triggered for - TC_05_RoutingDetailsDistributionReportValidations -with TestdataId: " + uniqueDataId);
		...
		for(int i=0;i<allMainRowsSize; i++)
		{
			...
			if(currentColumnMainData.size()==3)
			{
				String routePriorityMainUI = currentColumnMainData.get(0);
				String noOfRoutesMainUI = currentColumnMainData.get(1);
				String productMainUI = currentColumnMainData.get(2);
				
				//Drilldown
				test.log(LogStatus.INFO, "Entering Performance Drilldown for main index"+ String.valueOf(i+1)+" ^Product: "+productMainUI+" ^ Priority: "+routePriorityMainUI);
				...
				if(allDrilldownRowsSize == Integer.parseInt(noOfRoutesMainUI))
					test.log(LogStatus.PASS,
							"EXPECTECD: (DrillDown) No of routues in main table should match with number of rows in Drilldown table. No. of routes: "+allDrilldownRowsSize+". (main row index: "+ (i+1)+" ^ Product: "+productMainUI+" ^ Priority: "+routePriorityMainUI, 
				...
				if(drillDownSelectedProductUI.equals(productMainUI))
					test.log(LogStatus.PASS,
							"EXPECTECD: (DrillDown) Product in both Drilldown and Main table should be matched. (main row index: "+ (i+1)+" ^ Product: "+productMainUI+" ^ Priority: "+routePriorityMainUI,
				...
				for(int j=0;j<allDrilldownRowsSize;j++)
				{
					...
					if(currentColumnDrilldownData.size()==9)
					{
						String supplierAccountDrillDownUI = currentColumnDrilldownData.get(0);				
						String destinationDrillDownUI = currentColumnDrilldownData.get(1);
						String countryDrillDownUI= currentColumnDrilldownData.get(2);
						String priorityDrillDownUI = currentColumnDrilldownData.get(7);
						
						if(priorityDrillDownUI.equals(routePriorityMainUI))
							test.log(LogStatus.PASS,
									"EXPECTECD: (DrillDown) Priority in maintable and drilldowntable should be matched. (main row index: "+(i+1)+" ^ Product: "+productMainUI+" ^ Priority: "+routePriorityMainUI+" ^ Country: "+countryDrillDownUI+" ^ Destination: "+destinationDrillDownUI+" ^ SupplierAccount: "+supplierAccountDrillDownUI, 
									"Validation:  <span style='font-weight:bold;'>ACTUAL::Priority in maintable and drilldowntable is matching. Priority: "+routePriorityMainUI+"</span>");
						else
							test.log(LogStatus.FAIL,
									"EXPECTECD: (DrillDown) Priority in maintable and drilldowntable should be matched. (main row index: "+(i+1)+" ^ Product: "+productMainUI+" ^ Priority: "+routePriorityMainUI+" ^ Country: "+countryDrillDownUI+" ^ Destination: "+destinationDrillDownUI+" ^ SupplierAccount: "+supplierAccountDrillDownUI, 
									"Validation:  <span style='font-weight:bold;'>ACTUAL::Priority in maintable and drilldowntable is not matching. Maintable Priority: "+routePriorityMainUI+" Drilldown Priority: "+priorityDrillDownUI+"</span>");
					}
				}				
