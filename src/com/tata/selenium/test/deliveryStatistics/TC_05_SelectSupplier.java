...
public class TC_05_SelectSupplier implements ApplicationConstants {
	...
	public void DO(String uniqueDataId, String testCaseId) {
		...
		test.log(LogStatus.INFO, "Validating Table Headers in Supplier Main Page");	
		validateTableHeaders(dataMap.get("MainTableHeaders") , "Supplier Main Page");
		
		//CSV and UI table validation
		test.log(LogStatus.INFO, "Validating CSV and UI table data in Supplier Main Page");			
		if(!"MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSDeliveryStatistics.csv", "Supplier Main Page", 11);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSDeliveryStatistics.csv", "Supplier Main Page", 8);

		List<WebElement> supplierrAccElemets = driver.findElements(By.xpath("//*[@id='myTable']/tbody/tr/td[count(//*[@id='myTable']/thead/tr/th[text()='Supplier Account']/preceding-sibling::*)+1]/a"));
		...
		for (String suppAccVal : supplierAccValues) {
			mainPageAndSubsequenceDrillDownValidation(suppAccVal);
		}
	...
	void mainPageAndSubsequenceDrillDownValidation(String suppAccVal)
	{

			test.log(LogStatus.INFO, "$$$$$$$$$$$ Validating for supplier account: "+suppAccVal);
			Map<String, Double> uiDataMainPage = getUIValesMainPage(cu, suppAccVal);
	...
	void firstAndSubsequenceDrillDownValidation(String suppAccVal, Map<String, Double> uiDataMainPage)
	{
		...
		//Entering Supplier Account Distribution Page ( 1st drilldown)
		cu.clickElement("dynamicSupORCustAccORDestinationValue", "$SupplierORCustomerAccORDestinationVal$", suppAccVal);
		test.log(LogStatus.INFO, "######### Validating 1st drilldown ( Supplier Account Distribution Page )  for Supplier Account: "+suppAccVal);
		...
		validateTableHeaders(dataMap.get("FisrtDrilldownHeaders"), "Supplier Account Distribution Page");
		...
		exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSCustomerAccountDistribution.csv", "Customer Account Distribution Page", 6);
		...
		test.log(LogStatus.INFO, "Validate summation of CSV data (1st drilldown [ Supplier Account Distribution Page ]) with UI data (of main page) in  for Supplier Account: "+suppAccVal);
...
//		    try {
				...
				if(csvData.get(colName).equals(uiValue))					
					test.log(LogStatus.PASS,
							"EXPECTED: "
									+ "Validate the summation of csv values (1st drilldown) and the Ui Values (main page) of this Supplier account"
...
//			}
		...
		for(int i=0; i<allAttemptedFailureCount; i++)
		{
			if(i==0)
			{
				String suppNameDrill = cu.getAttribute("DrillDown_SupplierNameTxt", "value");
				String suppAccNameDrill = cu.getAttribute("DrillDown_SupplierAccNameTxt", "value");
	...
	{		
		...
		if("MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Attempted Failure Page", 8);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Attempted Failure Page", 9);
		...
		if(!"MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			...
			cu.checkElementNotPresence("DrillDown_CustomerAccNameTxt");	
		...
		if(!"MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			cusAcctName =cu.getAttribute("DrillDown_CustomerAccNameTxt", "value");
		String suppAccName = cu.getAttribute("DrillDown_SupplierAccNameTxt1", "value");
	...
	void secondDrillDownSubmitFailureValidation(int linkIndex)
	{
		...
		if("MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Submit Failure Page", 8);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Submit Failure Page", 9);
	...
	{			
		...
		if("MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Failed Page", 8);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSFailureDistribution.csv", "Failed Page", 9);
	...
	{	
		...
		if("MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSLatencyDistribution.csv", "Average Latency Drilldown Page", 6);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Supplier-SMSLatencyDistribution.csv", "Average Latency Drilldown Page", 8);
		...
		test.log(LogStatus.INFO, "Validating Sumation of total failure in 2nd drilldown ( Average Latency Drilldown Page )");
		String cusAcctName =null;
		if(!"MMX-Supplier Manager".equals(dataMap.get("UserRole").trim()))
			cusAcctName =cu.getAttribute("DrillDown_CustomerAccNameTxt", "value");
		String suppAccName = cu.getAttribute("DrillDown_SupplierAccNameTxt", "value");
		String customerSuppHierarchyString = "Supplier Account: "+suppAccName+(cusAcctName!=null?"-> Customer Account: "+cusAcctName:"");
