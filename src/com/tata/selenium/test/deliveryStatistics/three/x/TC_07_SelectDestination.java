...
public class TC_07_SelectDestination implements ApplicationConstants {
	...
	public void DO(String uniqueDataId, String testCaseId) {
		...
		//validate table headernames//Table header validation
		test.log(LogStatus.INFO, "Validating Table Headers in Destination Main Page");
		validateTableHeaders(dataMap.get("MainTableHeaders") , "Destination Main Page");

		List<WebElement> destinationElemets = driver.findElements(By.xpath("//*[@id='myTable']/tbody/tr/td[count(//*[@id='myTable']/thead/tr/th[text()='Destination']/preceding-sibling::*)+1]/a"));
		...
		for (String destinationVal : destinationValues) {
			mainPageAndSubsequenceDrillDownValidation(destinationVal);
		}
	...
	public Map<String, Integer> exportCSVAndGetCoverageFieldsUpdated()
    {
		...
		if("MMX-Customer Manager".equals(dataMap.get("UserRole").trim()) 
			...
			csvu= new CSVUtil(csvFilePath, 7);
	...
	void mainPageAndSubsequenceDrillDownValidation(String destinationVal)
	{	
		test.log(LogStatus.INFO, "$$$$$$$$$$$ Validating for Destination: "+destinationVal);
		Map<String, Double> uiDataMainPage = getUIValesMainPage(cu, destinationVal);
		...
		firstDrillDownValidation(destinationVal, uiDataMainPage);
	}
	
	void firstDrillDownValidation(String destinationVal, Map<String, Double> uiDataMainPage)
	{
		//Entering Destination Distribution Page ( 1st drilldown)
		cu.clickElement("dynamicSupORCustAccORDestinationValue", "$SupplierORCustomerAccORDestinationVal$", destinationVal);
		test.log(LogStatus.INFO, "######### Validating 1st drilldown ( Destination Distribution Page ) for Destination: "+destinationVal);
		...
		validateTableHeaders(dataMap.get("FisrtDrilldownHeaders"), "Destination Distribution Page");
		...
		if("MMX-Customer Manager".equals(dataMap.get("UserRole").trim()) 
				|| "MMX-Customer Finance".equals(dataMap.get("UserRole").trim()) 
					|| "MMX-Customer Routing".equals(dataMap.get("UserRole").trim()))
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Destination-SMSCustomerAccountDistribution.csv", "Destination Distribution Page", 6);
		else
			exportCSVAndValidateWithUI(getUITableDataAlongWithHeadersWithoutMapping(), "Destination-SMSCustomerAccountDistribution.csv", "Destination Distribution Page", 7);
		...
		test.log(LogStatus.INFO, "Validate summation of CSV data (1st drilldown [ Destination Distribution Page ]) with UI data (of main page) in  for Destination: "+destinationVal);
...
//		    try {
				...
				if(csvData.get(colName).equals(uiValue))					
					test.log(LogStatus.PASS,
							"EXPECTED: "
									+ "Validate the summation of csv values (1st drilldown) and the Ui Values (main page) of this Destination"
