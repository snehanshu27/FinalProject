...
//		cu.checkEditableDropDown("DeliveryStat_InstanceLst", dataMap.get("DeliveryStat_InstanceLst"));
		...
		WebElement drpEle = driver.findElement(By.xpath("//select[@id='custName']"));
		Select drpSele = new Select(drpEle);
		drpSele.deselectByVisibleText("ALL");
		drpSele.selectByVisibleText("COREGRAL HOLDING LTD");
...
//		cu.clickElement("DeliveryStatisticsPage");
		...
		List<WebElement> customerAccElemets = driver.findElements(By.xpath("//*[@id='myTable']/tbody/tr/td[2]/a"));
		...
		for (String cusAccVal : customerAccValues) {
			...
			Map<String, Double> uiData = getUIVales(cu, cusAccVal);
			...
			test.log(LogStatus.INFO, "######### Validating 1st drilldown for Customer account: "+cusAccVal);
					...
					if(csvData.get(colName).equals(uiValue))					
						test.log(LogStatus.PASS,
								"EXPECTED: " + cusAccVal
										...
										+ cusAccVal + " -> should be same. Filed Name : "+colName+"  - expected value: "+uiValue,
								"Validation:  <span style='font-weight:bold;'>ACTUAL::" + cusAccVal
										+ "The values of the csv data and the Ui Values of this customer account"
										+ cusAccVal + " are same. Filed Name : "+colName+"  - expected value: "+csvData.get(colName)+"</span>");
					else
						test.log(LogStatus.FAIL,
								"EXPECTED: " + cusAccVal
								+ "The values of the csv data and the Ui Values of this customer account"
						...
						"Validation:  <span style='font-weight:bold;'>ACTUAL::" + cusAccVal
								+ "The values of the csv data and the Ui Values of this customer account"
								+ cusAccVal + " are not same. Filed Name : "+colName+"  - expected value: "+csvData.get(colName)+"</span>");
//				} catch (Exception e) {
//					
...
//				}
			...
			for(int i=0; i<allAttemptedFailureCount; i++)
			{
				if(i==0)
				{
					String custNameDrill = driver.findElement(By.name("custNameDrill")).getAttribute("value");
					String custAccNameDrill = driver.findElement(By.name("custAccNameDrill")).getAttribute("value");
					test.log(LogStatus.INFO, "Validating Attempted Failure DrillDown Validations for Customer: "+custNameDrill+" -> CustomerAccount: "+custAccNameDrill);
				}
