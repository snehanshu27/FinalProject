...
public class TC_01_ProductProvisioningUIValidation implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_01_ProductProvisioningUIValidation.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) {
		...
		test = extent.startTest(
				"Execution triggered for - TC_01_ProductProvisioningUIValidation -with TestdataId: " + uniqueDataId);
		...
		cu.checkEditableBox("Product_Provisioning_ProductNameLst");
		
		// Validating all editable drop down
		cu.checkEditableDropDown("Product_Provisioning_PriorityLst", dataMap.get("Product_Provisioning_PriorityLst"));
		cu.checkEditableDropDown("Product_Provisioning_ServiceLst", dataMap.get("Product_Provisioning_ServiceLst"));
		cu.checkEditableDropDown("Product_Provisioning_ProductCurrencyLst", dataMap.get("Product_Provisioning_ProductCurrencyLst"));
		cu.checkEditableDropDown("Product_Provisioning_ProductStatusLst", dataMap.get("Product_Provisioning_ProductStatusLst"));

		//validating non editable text boxes
		cu.checkReadonlyProperty("Product_Provisioning_ProductIDTxt");
		cu.checkReadonlyProperty("Product_Provisioning_PValueTxt");
		
		// Validating all buttons
		cu.checkElementPresence("Product_Provisioning_PClassBtn");
		cu.checkElementPresence("Product_Provisioning_PListBtn");
		cu.checkElementPresence("Product_Provisioning_DisplayBtn");
		cu.checkElementPresence("Product_Provisioning_SubmitBtn");
		cu.checkElementPresence("Product_Provisioning_CancelBtn");
		cu.checkElementPresence("Product_Provisioning_ExportBtn");
		
		
		// Taking screenshot and Logging out
		cu.getScreenShot("ScreenShot of Validation Popped Up");
