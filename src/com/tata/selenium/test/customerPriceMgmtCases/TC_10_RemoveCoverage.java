...
public class TC_10_RemoveCoverage implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_10_RemoveCoverage.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest("Execution triggered for - TC_10_RemoveCoverage -with TestdataId: " + uniqueDataId);
		...
		if (("MT SMS").equalsIgnoreCase(dataMap.get("ServiceNameLst"))) {
			cu.unSelectCheckBox("dynamicCoverageChk", "$destination$", dataMap.get("dynamicDestination"));
		} else {
			cu.unSelectCheckBox("dynamicCoverageChk", "$destination$", dataMap.get("dynamicCountry"));
		}
		cu.clickElement("SubmitBtn");
		cu.checkMessage("application_PopUpMessage", "Click submit",
