...
public class TC_11_ValidateBlockedStatusInBothUIAndExportedExcel implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_11_ValidateBlockedStatusInBothUIAndExportedExcel.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent
				.startTest("Execution triggered for - TC_11_ValidateBlockedStatusInExportedExcel -with TestdataId: "
						+ uniqueDataId);
		...
		if("Y".equalsIgnoreCase(dataMap.get("Product_Routing_BlockedChk")))
			cu.checkCheckBoxSelected("Product_Routing_SupplierAccount_Blocked_CheckBox", "$CountryName$~$DestinationName$~$SupplierAccName$", dataMap.get("Product_CountryLst")+"~"+dataMap.get("Product_DestinationLst")+"~"+dataMap.get("SupplierAccName"));
		else
			if("N".equalsIgnoreCase(dataMap.get("Product_Routing_BlockedChk")))
				cu.checkCheckBoxUnselected("Product_Routing_SupplierAccount_Blocked_CheckBox", "$CountryName$~$DestinationName$~$SupplierAccName$", dataMap.get("Product_CountryLst")+"~"+dataMap.get("Product_DestinationLst")+"~"+dataMap.get("SupplierAccName"));
	...
	public void exportCSVAndCoverageFieldsUpdated(CommonUtils1 cu, Map<String, String> dataMap) throws Exception {
		...
		if (csvFilePath.trim().contains(expectedFileName.trim()))
			test.log(LogStatus.PASS,
					"EXPECTECD: Exported file name should be in 'Product_NameLst.csv' - '" + expectedFileName + "'",
		...
		{
			if(desRow.get("Supplier Account").equals(dataMap.get("SupplierAccName")))
				if(!desRow.get("Blocked").equalsIgnoreCase(dataMap.get("Product_Routing_BlockedChk")))
				{
					test.log(LogStatus.FAIL,
							"EXPECTECD: In the Exported file, the status of the Blocked column for Destination: "+dataMap.get("Product_DestinationLst")+" > SupplierAccName: "+dataMap.get("SupplierAccName")+" should be changed to "+dataMap.get("Product_Routing_BlockedChk"),
							"Usage: <span style='font-weight:bold;'>ACTUAL:: In the Exported file, the status of the Blocked column has not changed to "+dataMap.get("Product_Routing_BlockedChk")+" ( vaule is still "+desRow.get("Blocked")+" )</span>");
				}
				else{
					test.log(LogStatus.PASS,
							"EXPECTECD: In the Exported file, the status of the Blocked column for Destination: "+dataMap.get("Product_DestinationLst")+" > SupplierAccName: "+dataMap.get("SupplierAccName")+" should be changed to "+dataMap.get("Product_Routing_BlockedChk"),
							"Usage: <span style='font-weight:bold;'>ACTUAL:: In the Exported file, the status of the Blocked column has been changed to "+desRow.get("Blocked")+"  </span>");
				}
		}
