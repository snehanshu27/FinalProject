...
public class TC_04_PayableValidateDataMobileLNS implements ApplicationConstants {

	private static final Logger LOGGER = Logger.getLogger(TC_04_PayableValidateDataMobileLNS.class.getName());
	...
	public void DO(String uniqueDataId, String testCaseId)  {
		...
		test = extent
				.startTest("Execution triggered for - TC_04_PayableValidateDataMobileLNS -with TestdataId: " + uniqueDataId);
		...
		//String accStatus = cu.getText("dynamicAccStatus", "$SupplierAcc$", dataMap.get("dynamicEffectiveDate"));
		//String product = cu.getText("dynamicProduct", "$SupplierAcc$", dataMap.get("dynamicEffectiveDate"));
		...
		if (cu.existsElement("Unique_Routing_Row", "$SupplierAccName$~$EffectiveDate$", dataMap.get("SupplierAccName")+"~"+dataMap.get("dynamicEffectiveDate"))) {
			...
			if (dataMap.get("SupplierAccName").equalsIgnoreCase(custAccName)) {
				test.log(LogStatus.PASS,
						...
						"Validation:  <span style='font-weight:bold;'>ACTUAL::  SupplierAccName is displayed as - "
								+ custAccName + "</span>");
			} else {
	...
	public void exportCSVFieldsUpdated(CommonUtils cu, Map<String, String> dataMap)  {
		...
		if (csvDatamap.get("Country").equals(dataMap.get("dynamicCountry"))
				...
				&& csvDatamap.get("Cost").equals(dataMap.get("Price"))
