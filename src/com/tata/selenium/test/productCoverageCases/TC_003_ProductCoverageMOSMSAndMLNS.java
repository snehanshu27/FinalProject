...
public class TC_003_ProductCoverageMOSMSAndMLNS implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_003_ProductCoverageMOSMSAndMLNS.class.getName());
	...
	public void DO (String uniqueDataId, String testCaseId) throws Exception {
		...
		test = extent.startTest("Execution triggered for - TC_003_ProductCoverageMOSMSAndMLNS -with TestdataId: "+uniqueDataId);
	...
	@AfterMethod(dependsOnMethods="afterMethodFailed")
	  ...
	  public void validateHistory( Map<String, String> dataMap, CommonUtils cu, String selectedAccName) throws Exception
	  {
			...
			else
			{
				if(coverageHistories.size() ==6 || coverageHistories.size() <7)
				{
					...
					{					
						...
						cu.clickElement("Product_Coverage_DisplayBtn");
						cu.waitForPageLoad("");
						String disAtt = cu.getAttribute("dynamicCoverageCheckbox_ByCountry", "disabled", "$countryVal$", dataMap.get("Country").split("\\~")[0]);
	  ...
	  public void modifyCoverage(CommonUtils cu, Map<String, String> dataMap)
	  {
		  ...
		  {
			  if("Y".equalsIgnoreCase(coverageSplt[i]))			
					cu.selectCheckBox("dynamicCoverageCheckbox_ByCountry", "$countryVal$", countrySplt[i]);			
				else
					cu.unSelectCheckBox("dynamicCoverageCheckbox_ByCountry", "$countryVal$", countrySplt[i]);
		  }
			//click submit
	  ...
	  {	
		  ...
		  for(int i=1;i<countrySplt.length; i++)
		  {
				if("Y".equalsIgnoreCase(dataMap.get("Coverage")))
					cu.checkCheckBoxSelected("dynamicCoverageCheckbox_ByCountry", "$countryVal$", countrySplt[i]);
				else{
					if("N".equalsIgnoreCase(dataMap.get("Coverage")))
						cu.checkCheckBoxUnselected("dynamicCoverageCheckbox_ByCountry", "$countryVal$", countrySplt[i]);
				}			
				
				
				/*if("Y".equalsIgnoreCase(dataMap.get("SC")))
					cu.checkCheckBoxSelected("dynamicShortCheckboxSC_ByCountry", "$countryVal$", countrySplt[i]);
				else{
					if("N".equalsIgnoreCase(dataMap.get("SC")))
						cu.checkCheckBoxUnselected("dynamicShortCheckboxSC_ByCountry", "$countryVal$", countrySplt[i]);
				}
				*/
				/*if("Y".equalsIgnoreCase(dataMap.get("LN")))
					cu.checkCheckBoxSelected("dynamicLongCheckboxLN_ByCountry", "$countryVal$", countrySplt[i]);
				else{
					if("N".equalsIgnoreCase(dataMap.get("LN")))
						cu.checkCheckBoxUnselected("dynamicLongCheckboxLN_ByCountry", "$countryVal$", countrySplt[i]);
				}*/
		  }
	  ...
	  public Map<String, String> getCurrentCoverageUIStatus(CommonUtils cu, String country) throws Exception 
	  {
		  ...
		  {
			  LOGGER.info(" Get webtable coverage data for Country: :: Exception:"+e);
	  ...
	  public void checkInputAndUICoverageIsNotSame(CommonUtils cu, Map<String, String> dataMap) throws Exception
	  {
		  String[] countrySplt = dataMap.get("Country").split("\\~");
		  String[] coverageSplt = dataMap.get("Coverage").split("\\~");
		 // String[] SCSplt = dataMap.get("SC").split("\\~");
	  ...
	  public void checkInputAndUICoverageIsSame(CommonUtils cu, Map<String, String> dataMap) throws Exception
	  {
							...
							&& uiData.get("DR").equalsIgnoreCase(dataMap.get("DR"))))*/
			{
				...
				test.log(LogStatus.FAIL, "EXPECTECD: Input sheet and UI coverage data should be same, After changed were made", "Usage: <span style='font-weight:bold;'>ACTUAL:: Input sheet and UI coverage data is not same, data is  UI_Coverage: "+uiData.get("Coverage")+" ?? Data_Coverage: "+dataMap.get("Coverage")
																																																									+"\n  UI_AN: "+uiData.get("AN")+" ?? Data_AN: "+dataMap.get("AN")
																																																									+"\n  UI_SC: "+uiData.get("SC")+" ?? Data_SC: "+dataMap.get("SC")
																																																									+"\n  UI_LN: "+uiData.get("LN")+" ?? Data_LN: "+dataMap.get("LN")
