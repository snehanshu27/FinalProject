...
public class TC_006_DeleteAllErrorCode implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_006_DeleteAllErrorCode.class.getName());
	...
	public void DO (String uniqueDataId, String testCaseId) throws InterruptedException {
		...
		if(totalrows>0)
		{
			hu.getScreenShot("Total rows before delete");
			for(int i=0;i<totalrows;i++)
			{
				//click delete button of firstrow
				hu.clickElement("customErrorCode_TableFirstRow_DeleteButton");
				hu.sleep(500);
				
				//Check Popup message and confirm
				hu.checkMessage("application_PopUpMessage", "click display button. No record found", "Warning: This action will delete the current row. Do you want to Continue ?");
				hu.sleep(500);
				hu.waitForPageLoad("");
			}
			
			hu.getScreenShot("Delete Row without clicking submit button");
			...
			//Check Popup message and confirm
			hu.checkMessage("application_PopUpMessage", "Confirm the submit action", "Warning: This action will submit the record without any custom error definition and create an order. Do you want to Continue?");		
			hu.sleep(500);
			hu.waitForPageLoad("");
		...
		else
		{
			...
			LOGGER.error("No record found. Atleast one row should be presenet for delete validation");
			test.log(LogStatus.FAIL, "No Record found", "No record found. Atleast one row should be presenet for delete validation");
		}
