...
public class TC_005_DeleteErrorCode implements ApplicationConstants {
	private static final Logger LOGGER = Logger.getLogger(TC_005_DeleteErrorCode.class.getName());
	...
	public void DO (String uniqueDataId, String testCaseId) throws InterruptedException {
		...
		if(hu.elementDisplayed("dynamicCustomErrorCode_ColText", "$errorcode$", dataMap.get("ErrorCode")))
		{
			...
			hu.clickElement("dynamicCustomErrorCode_DeleteButton", "$errorcode$", dataMap.get("ErrorCode"));
			hu.sleep(1000);
			...
			boolean firstRowPresent = hu.elementDisplayed("customErrorCode_TableFirstValidRow", 3);
		...
		{			
			LOGGER.error("Unable to find a record/row with error code - "+dataMap.get("ErrorCode"));
			test.log(LogStatus.FAIL, "Error code Delete valiadtion",
